...
# Load data files
expense_feed = pd.read_csv('raw_office_expense_feed_500.csv')
re_feed = pd.read_csv('raw_real_estate_feed_500.csv')

# Functions created to clean zip codes and phone numbers
def clean_zip(df, col):
    if col in df.columns:
        df[col] = df[col].astype(str).str.replace(r'\.0$', '', regex=True).str.zfill(5)
    return df

def clean_phone(df, col):
    if col in df.columns:
...
neighborhood_df = clean_zip(neighborhood_df, 'zip_code')
...
offices_df = clean_zip(offices_df, 'zip_code')
offices_df = clean_phone(offices_df, 'phone_number')
...
employees_df = clean_phone(employees_df, 'phone_number')
...
properties_df = clean_zip(properties_df, 'zip_code')
...
clients_df = clean_phone(clients_df, 'phone_number')
...
client_requirements_df = clean_zip(client_requirements_df, 'zip_code')
...
transactions_df['client_role'] = transactions_df['client_role'].str.lower()
...
schools_df = clean_zip(schools_df, 'zip_code')
...
# public_transit
public_transit_df = re_feed[['station_id', 'neighborhood_zip_code', 'transit_type']].rename(columns={'station_id': 'stationid', 'neighborhood_zip_code': 'zip_code'}).drop_duplicates(subset=['stationid']).dropna(subset=['stationid'])
public_transit_df = clean_zip(public_transit_df, 'zip_code')
